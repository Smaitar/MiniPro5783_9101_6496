    ...
    private static void simulationProgress()
    {
        ...
        while (!s_flagStopSimulation)
        {
            ...
            else
            {
                ...
                order = order.Status == OrderStatus.Confirmation ? _bl.Order.UpdateSentOrder(order.ID)
                        : _bl.Order.UpdateSuppliedOrder(order.ID);

                items =
                     Tuple.Create(order, (order.Status == OrderStatus.Sent ? (OrderStatus?)OrderStatus.Supplied : null), "", "");
