    ...
    private static void s_InitializOrder()
    {
        ...
        for (int i = 0; i < NumOrder; i++)
        {
            Order order = new Order();
            {
                ...
                {
                    order.ShipDate = order.OrderDate + new TimeSpan(random.Next(60), random.Next(60), random.Next(24));

                    if (i < 40)
                        order.DeliveryDate = null;
                    else
                        order.DeliveryDate = order.ShipDate + new TimeSpan(random.Next(60), random.Next(60), random.Next(24));
                }
            }
