...
internal class Cart : ICart
{
    ...
    public BO.Cart UpdateCart(BO.Cart cart, int id, int amount)
    {
        ...
        if (amount < 0)
            throw new NagtiveNumberException("negative amount");
        int index;
        try
        {
            index = cart.Items.FindIndex(x => x!.ProductID == id);
        ...
        if (cart.Items[index]!.Amount < amount)
        {
            ...
            cart.Items[index]!.Price = cart.Items[index]!.Amount * product.Price;
    ...
    public bool AprrovedCart(BO.Cart cart)
    {
        ...
        if (cart.Items.Any())
        {
            ...
            if (cart.CustomerEmail == null || !new EmailAddressAttribute().IsValid(cart.CustomerEmail))
